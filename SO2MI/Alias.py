...
from .Exceptions import (NameDuplicationError, NoItemError,
    ...
    if os.path.isfile("alias.json"):
        ...
        try:
            ...
            logger("alias.jsonの構文にエラーがあります\n行: {0} 位置: {1}\n{2}".format(
                exc.lineno, exc.pos, exc.msg), "error")
            return itemName
    else:
    ...
    if os.access("alias.json", os.W_OK):
        if os.path.isfile("alias.json"):
            ...
            if aliasName in allAlias:
                raise SameAliasNameExistError(
                    "existent of the same alias name")

            # 正式名称がアイテムに存在しなければNoItemErrorを返す
            item = getApi(
                "item", "https://so2-api.mutoys.com/master/item.json")
            recipe = getApi(
                "recipe", "https://so2-api.mutoys.com/json/master/recipe_item.json")
            itemId = 0
            for col in item:
